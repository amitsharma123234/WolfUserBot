...
#
""" WolfUserBot module for getting information about the server. """
...
import psutil
from wolfuserbot import CMD_HELP, ALIVE_NAME, BOT_VER, ALIVE_LOGO, bot, StartTime
...
async def amireallyalive(alive):
    ...
    output = (f"`MADE IN 🇮🇳 , MADE FOR 🐺`\n"
             f"`тєℓєтнση νєяѕιση ♥️`: {version.__version__} \n"
             f"`ρутнση νєяѕιση 🐍`: {python_version()} \n"
             f"`вσт νєяѕιση 🐺: UserBot {BOT_VER}` \n"
             f"==================================== \n"
             f"`υѕєя 👦🏻`: {DEFAULTUSER} \n"
             f"`мαιηтαιηєя ⚙️`: [┃🅡@🅙🅥€€🅡┃](t.me/Smart_S54) \n"
             f"`вσт υρтιмє ⏱️`: {uptime} \n"
             f"`Wolf UserBot Support`: @Wolf_User_Bot \n"
             f"====================================\n")
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
...
\n\n`.botver`\
\nUsage: Shows the wolfuserbot version.\
\n\n`.pip` <module(s)>\

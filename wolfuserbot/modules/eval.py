...
#
""" WolfUserBot module for executing code and terminal commands from Telegram. """

import asyncio
from os import remove
from sys import executable
from wolfuserbot import CMD_HELP, BOTLOG, BOTLOG_CHATID, TERM_ALIAS
...
async def evaluate(query):
    ...
    if expression in ("wolfuserbot.session", "config.env"):
...
async def run(run_q):
    ...
    if code in ("wolfuserbot.session", "config.env"):
...
async def terminal_runner(term):
    ...
    if command in ("wolfuserbot.session", "config.env"):

...
# Welcome Message
welcome() {
    echo "*****Welcome to wolfuserbot setup*****
This Guided Setup shall help you get your own wolfuserbot up and running.
...
botclone() {
    ...
    if [ -z "$bot_clone" ]; then
        git clone https://github.com/WolfGangIndia/WolfUserBot -b sql-extended
        save_prog "bot_clone"
    fi
    echo "DONE!!"
...
#Generate the wolfuserbot.session
session() {
    echo "Generating session..."
    if [ -z "$sess" ]; then
        python3 windows_startup_script.py
        python3.8 -m wolfuserbot test
        save_prog "sess"
    fi
    echo "DONE!!"
}
...
systemd() {
    echo "Sys service..."
    if [ -z "$sysserv" ]; then
        sudo mv wolfuserbot /etc/systemd/system/wolfuserbot.service
        save_prog "sysserv"
    fi
    sudo systemctl start wolfuserbot.service
    sudo systemctl enable wolfuserbot.service
    echo "DONE!!"
}
